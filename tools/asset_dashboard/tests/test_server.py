...
def test_stage_returns_images_and_prompt(client, tmp_path, monkeypatch):
            ...
            "alice": {
                ...
                "stages": {
                    "generate_8dir_base": {
                        ...
                        "paths": [
                            "art_source\\characters\\alice\\rotations\\south.png",
                            "art_source\\characters\\alice\\rotations\\east.png",
                        ],
                    },
                },
    ...
    assert data["images"][0]["path"] == "art_source/characters/alice/rotations/south.png"
...
def test_file_serves_existing_png(client, tmp_path, monkeypatch):
    ...
    fake_png = tmp_path / "art_source/characters/alice/rotations/south.png"
    fake_png.parent.mkdir(parents=True)
    fake_png.write_bytes(b"\x89PNG\r\n\x1a\nfake")
    monkeypatch.setattr(server, "REPO_ROOT", tmp_path)
    r = client.get("/api/asset/file", params={"p": "art_source/characters/alice/rotations/south.png"})
    assert r.status_code == 200, r.text
...
def test_file_rejects_path_traversal(client, tmp_path, monkeypatch):
    monkeypatch.setattr(server, "REPO_ROOT", tmp_path)
    r = client.get("/api/asset/file", params={"p": "art_source/../../../etc/passwd"})
    assert r.status_code in (400, 403, 404)


def test_file_missing_404(client, tmp_path, monkeypatch):
    monkeypatch.setattr(server, "REPO_ROOT", tmp_path)
    (tmp_path / "art_source").mkdir(parents=True)
    r = client.get("/api/asset/file", params={"p": "art_source/ghost.png"})
    assert r.status_code == 404
