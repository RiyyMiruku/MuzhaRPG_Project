...
使用者
    ...
    │
    └── art_source/pipeline/orchestrators/   (CLI — 等值 raw access)
            │
...
| **CLI Orchestrator** (`art_source/pipeline/orchestrators/*.py`) | 可在終端機直接跑；支援 `--resume-from`、`--review-mode none` 批次模式；AI 透過 Bash 工具調用 |
