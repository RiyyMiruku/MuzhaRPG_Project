...
# → 看 pipeline/output/characters/chen_ayi/rotations/*.png
...
### Q: 我要把資產從一章節改到另一章節
A: 改 manifest 的 `tags`,把 `chapter:1` 改成 `chapter:2`。或在 dashboard 上 — 目前 UI 還不支援 tag 編輯,直接改 `pipeline/output/manifest.json` 也行(它是 single source of truth)。
